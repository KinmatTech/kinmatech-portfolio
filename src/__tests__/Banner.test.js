...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Ani Mathew O.' is in the banner component
	expect(screen.getByText(/Hi, I am Ani Mathew Okechukwu/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Mathew-CV.pdf');
	// expect(downloadLink.href).toEqual('/files/Mathew-CV.pdf');
